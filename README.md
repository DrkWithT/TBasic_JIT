...
### Usage
 ...
 2. Enable `./project.sh`.
 3. Run `./project.sh build debug-build <your build tool>`. The executable will be at path `./build/<debug | release | profile>/toyjit`.
...
### Design Basics
 ...
 - Each specialized operation has a type guard that may jump to a `deopt:` region of native code which calls a helper function to rollback VM state to before the function call.
 - Singly pass over a view of a hot bytecode chunk to compile a native code stub.
 - Native code stubs are trampolined. Trampolines are short bytecode chunks that have `{NATIVE_CALL <JIT-stub-ID>, RET}, {<original constant copies...>}` that wrap the JIT call to avoid extra branching per general call.

### Design of Deopt -- **TODO**
 - **Purpose:** cleanly recover if a type-specialized stub cannot be run due to any reason.
   - During interpretation of the trampoline, if a type guard is failed before any JIT call, know the original VM state to fall back to. Only allow this bail-out up to **3x**.
 - **Consequences - Type Guard Failure of Arguments**
   - The deopt logic runs via helper under label `deopt:`, removing the VM trampoline's `CallFrame` and transitioning the VM to a slow, generic path through the original bytecode chunk.
   - An `oops` tagged Value indicates type guard failure so we can deopt.
   - The call sites in the enclosing caller's bytecode are reverted too.

### TODOs
 1. Add deoptimization logic as per above notes.
 2. Add multiplication & division JIT support.
 3. Add initial, modifed _TBasic_ dialect frontend & bytecode compiler.
    - Support to TBasic `0.5.0` and diverge from there.
